...
export default function AdminPage() {
  ...
  return <main className="dashboard"><aside><div className="brand light"><span className="brand-mark">P</span><span>Pixellar <b>Spaces</b></span></div><nav><button className={tab==="leads"?"active":""} onClick={()=>setTab("leads")}>Enquiries & visits <b>{newCount}</b></button><button className={tab==="properties"?"active":""} onClick={()=>setTab("properties")}>Properties <b>{properties.length}</b></button><Link href="/admin/owners">Owner submissions →</Link></nav><Link href="/">View public website ↗</Link><button className="logout" onClick={logout}>Sign out</button></aside><section className="dash-main"><header><div><span className="kicker">OPERATIONS CENTER</span><h1>{tab==="leads"?"Leads & visits":"Property inventory"}</h1></div>{tab==="properties"&&<button className="orange-btn" onClick={()=>openPropertyForm(null)}>+ Add property</button>}</header>
